        ...
        <div id = "panel_comment">
            <table class="table table-bordered table-scrollable">
                ...
                <tr>
                    ...
                    <th scope="col" class="base-table-header">댓글내용</th>
                    <th scope="col" class="base-table-header">신고자</th>
                ...
                if($cnt == 0) {
                    ...
                    foreach ($arrDeclareComment as $row) {
                        ...
                        echo '<td><select class="combobox" id="commentStatus" onchange="onChangeStatus('.$row['id'].', this)">';
                        if($row['freetalkcommentStatus'] == STATUS_LIFE) {
                            echo '<option value="' . STATUS_LIFE . '" selected> 일반 </option>';
                        }
                        else {
                            echo '<option value="' . STATUS_LIFE . '"> 일반 </option>';
                        }
                        if($row['freetalkcommentStatus'] == STATUS_DELETE) {
                            echo '<option value="' . STATUS_DELETE . '" selected> 삭제 </option>';
                        }
                        else {
                            echo '<option value="' . STATUS_DELETE . '"> 삭제 </option>';
                        }
                        if($row['freetalkcommentStatus'] == STATUS_HIDE) {
...
<script>
    ...
    function searchKeyPress(e)
    {
        ...
        if (e.keyCode == 13)
        {
            ...
            location.href = 'declare_comment_page?page_num='+currentPage;
            return false;
        }
        return true;
    }
